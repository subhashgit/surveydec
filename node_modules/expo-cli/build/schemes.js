...
async function getSchemesForIosAsync(projectRoot) {
  try {
    const configPath = _config().IOSConfig.Paths.getInfoPlistPath(projectRoot);

    const rawPlist = fs().readFileSync(configPath, 'utf8');

    const plistObject = _plist().default.parse(rawPlist);

    return _config().IOSConfig.Scheme.getSchemesFromPlist(plistObject);
  } catch (_unused) {
...
async function getSchemesForAndroidAsync(projectRoot) {
  try {
    const configPath = await _config().AndroidConfig.Paths.getAndroidManifestAsync(projectRoot);
    const manifest = await _config().AndroidConfig.Manifest.readAndroidManifestAsync(configPath);
    return await _config().AndroidConfig.Scheme.getSchemesFromManifest(manifest);
  } catch (_unused2) {
