...
async function configureUpdatesAsync(projectDir, exp) {
  ...
  let xcodeProject = _config().IOSConfig.XcodeUtils.getPbxproj(projectDir);

  if (!_config().IOSConfig.Updates.isShellScriptBuildPhaseConfigured(projectDir, exp, xcodeProject)) {
    xcodeProject = _config().IOSConfig.Updates.ensureBundleReactNativePhaseContainsConfigurationScript(projectDir, exp, xcodeProject);
    await _fsExtra().default.writeFile(_config().IOSConfig.Paths.getPBXProjectPath(projectDir), xcodeProject.writeSync());
  }

  let expoPlist = await readExpoPlistAsync(projectDir);

  if (!_config().IOSConfig.Updates.isPlistConfigurationSynced(exp, expoPlist, username)) {
...
async function syncUpdatesConfigurationAsync(projectDir, exp) {
  ...
  if (!_config().IOSConfig.Updates.isPlistVersionConfigurationSynced(exp, expoPlist)) {
    expoPlist = _config().IOSConfig.Updates.setVersionsConfig(exp, expoPlist);
    await writeExpoPlistAsync(projectDir, expoPlist);
  }
...
async function ensureUpdatesConfiguredAsync(projectDir, exp) {
  const xcodeProject = _config().IOSConfig.XcodeUtils.getPbxproj(projectDir);
  ...
  if (!_config().IOSConfig.Updates.isPlistConfigurationSet(expoPlist)) {
    throw new Error('Missing values in Expo.plist');
  }
}

async function readExpoPlistAsync(projectDir) {
  const expoPlistPath = _config().IOSConfig.Paths.getExpoPlistPath(projectDir);
